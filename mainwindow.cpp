    ...
    : QMainWindow(parent), _project()
{
    setupUi(this);
	//GMSH
	//connect(action3D, SIGNAL(triggered()), this, SLOT(gmsh3D()));
	//Delaunary
	connect(actionDelaunay, SIGNAL(triggered()), this, SLOT(showDelaunay()));
    ...
    //modellingTabWidget->treeView->setModel(_processModel.get());  LCQ_CHANGE
    ...
    //tabifyDockWidget(mshDock, modellingDock);  LCQ_CHANGE
    //tabifyDockWidget(modellingDock, stationDock); LCQ_CHANGE
    ...
    //QAction* showModellingDockAction = modellingDock->toggleViewAction();  LCQ_CHANGE
    //showModellingDockAction->setStatusTip(tr("Shows / hides the Process view"));  LCQ_CHANGE
    /*connect(showModellingDockAction, SIGNAL(triggered(bool)), this,
            SLOT(showConditionDockWidget(bool)));
    menuWindows->addAction(showModellingDockAction);*/  //LCQ_CHANGE
...
void MainWindow::gmsh2D()
{
	
	QString filter;
	filter = "point file (*.geo)";

	QDir dir;
	QString fileName = QFileDialog::getOpenFileName(this, QString(tr("打开文件")), dir.absolutePath(), filter);
	if (fileName.isEmpty() == true) return;
	QFileInfo fi(fileName);
	QString fi_path , fi_name;
	fi_path = fi.absoluteFilePath();
	fi_name = fi.fileName();
	QString outfi_name,outfi_path;
	outfi_name = "2D" + fi_name.mid(0, fi_name.indexOf (".")) + ".msh";
	outfi_path = fi_path.mid(0, fi_path.lastIndexOf("/")) + "/" + outfi_name;
	//QProcess process(0);
	//QStringList arguements;
	QString comm = "gmsh " + fi_path + " -2" + " -o " + outfi_path;
	std::string str = comm.toStdString();
	const char* ch = str.c_str();
	//arguements<<"gmsh";
	//process.start("cmd",arguements);
	system(ch);
	loadFile(ImportFileType::GMSH, outfi_path);



}
void MainWindow::gmsh3D()
{
	...
	fi_name = fi.fileName();
	QString outfi_name, outfi_path;
	outfi_name = "3D" + fi_name.mid(0, fi_name.indexOf(".")) + ".msh";
	outfi_path = fi_path.mid(0, fi_path.lastIndexOf("/")) + "/" + outfi_name;
	//QProcess process(0);
	//QStringList arguements;
	QString comm = "gmsh " + fi_path + " -3" + " -o " + outfi_path;;
	std::string str = comm.toStdString();
	const char* ch = str.c_str();
	//arguements<<"gmsh";
	//process.start("cmd",arguements);
...
void MainWindow::showDelaunay()
{
	...
	triangulatedActor->GetProperty()->SetColor(1, 0, 0);
	...
	visualizationWidget->vtkWidget->GetRenderWindow()->AddRenderer(renderer);
	
	vtkSmartPointer<vtkRenderWindowInteractor> renderWindowInteractor = vtkSmartPointer<vtkRenderWindowInteractor>::New();
	renderWindowInteractor->SetRenderWindow(visualizationWidget->vtkWidget->GetRenderWindow());

	vtkSmartPointer<VtkCustomInteractorStyle>_interactorStyle = vtkSmartPointer<VtkCustomInteractorStyle>::New();
	
	visualizationWidget->vtkWidget->GetRenderWindow()->GetInteractor()->SetInteractorStyle(_interactorStyle);
	
	renderer->ResetCamera();
	renderer->DrawOn();
	visualizationWidget->vtkWidget->GetRenderWindow()->Render();
...
{
    QString about("<a href='https://www.ouc.edu.cn'>https://www.ouc.edu.cn</a><br /><br />");
    /*about.append(QString("Version: <a href='https://www.ouc.edu.cn'>%1</a><br /><br />")
        .arg(QString::fromStdString(BaseLib::BuildInfo::git_describe))
        .arg(QString::fromStdString(BaseLib::BuildInfo::ogs_version)));*/

    /*about.append(QString("Git commit: <a href='https://www.ouc.edu.cn'>%1</a><br />")
    ...
    QMessageBox::about(this, "About OUC", about);
}
...
QMenu* MainWindow::createImportFilesMenu()
{
    ...
    QMenu* importFiles = new QMenu("&导入", this);
